...
async def _browser_intercept_request(
    ctx: ToolContext, args: BrowserInterceptRequestArgs,
) -> dict[str, Any]:
    drv = _require_browser_driver(ctx)
    return await _invoke_browser(
        ctx, action_type="browser.intercept_request", target_uri=args.url_pattern,
...
async def _browser_mock_response(
    ctx: ToolContext, args: BrowserMockResponseArgs,
) -> dict[str, Any]:
    drv = _require_browser_driver(ctx)
    return await _invoke_browser(
        ctx, action_type="browser.mock_response", target_uri=args.url_pattern,
        args_summary={"status": args.status, "body_len": len(args.body)},
        coro_factory=lambda: drv.mock_response(
            args.url_pattern, args.body, status=args.status,
...
async def _browser_block_url_pattern(
    ctx: ToolContext, args: BrowserBlockUrlPatternArgs,
) -> dict[str, Any]:
    drv = _require_browser_driver(ctx)
    return await _invoke_browser(
        ctx, action_type="browser.block_url_pattern", target_uri=args.url_pattern,
...
async def _browser_wait_for_response(
    ctx: ToolContext, args: BrowserWaitForResponseArgs,
) -> dict[str, Any]:
    drv = _require_browser_driver(ctx)
    ...
    return await _invoke_browser(
        ctx, action_type="browser.wait_for_response", target_uri=args.url_pattern,
        args_summary={"timeout": args.timeout}, coro_factory=_wait,
    )


async def _browser_get_network_log(
    ctx: ToolContext, args: BrowserGetNetworkLogArgs,
) -> dict[str, Any]:
    drv = _require_browser_driver(ctx)
    ...
    return await _invoke_browser(
        ctx, action_type="browser.get_network_log", target_uri=None,
...
def build_browser_network_tools() -> list[ToolSpec[Any]]:
    return [
        ToolSpec(name="browser_intercept_request",
                 description="Route requests matching url_pattern; log or block.",
                 args_model=BrowserInterceptRequestArgs, handler=_browser_intercept_request, defer_loading=True),  # noqa: E501
        ToolSpec(name="browser_mock_response",
                 description="Mock a response for url_pattern with body/status/content_type.",
                 args_model=BrowserMockResponseArgs, handler=_browser_mock_response, defer_loading=True),  # noqa: E501
        ToolSpec(name="browser_block_url_pattern",
                 description="Block all requests matching url_pattern.",
                 args_model=BrowserBlockUrlPatternArgs, handler=_browser_block_url_pattern, defer_loading=True),  # noqa: E501
        ToolSpec(name="browser_wait_for_response",
                 description="Wait until a response matches url_pattern; returns metadata.",
                 args_model=BrowserWaitForResponseArgs, handler=_browser_wait_for_response, defer_loading=True),  # noqa: E501
        ToolSpec(name="browser_get_network_log",
                 description="Read the buffered network request log captured by interceptors.",
                 args_model=BrowserGetNetworkLogArgs, handler=_browser_get_network_log, defer_loading=True),  # noqa: E501
    ]
