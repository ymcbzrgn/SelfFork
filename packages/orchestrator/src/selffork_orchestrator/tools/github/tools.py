...
async def _github_repo_list(ctx: ToolContext, args: GithubRepoListArgs) -> dict[str, Any]:
    ...
    return await _invoke_gh(
        ctx,
        action_type="github.repo_list",
        target_uri=args.owner,
...
async def _github_repo_view(ctx: ToolContext, args: GithubRepoViewArgs) -> dict[str, Any]:
    ...
    return await _invoke_gh(
        ctx,
        action_type="github.repo_view",
        target_uri=args.repo,
        args_summary={"repo": args.repo},
        cmd=cmd,
        timeout=30.0,
    )
...
async def _github_repo_create(ctx: ToolContext, args: GithubRepoCreateArgs) -> dict[str, Any]:
    ...
    return await _invoke_gh(
        ctx,
        action_type="github.repo_create",
        target_uri=args.name,
        args_summary={
            "name": args.name,
...
async def _github_issue_create(ctx: ToolContext, args: GithubIssueCreateArgs) -> dict[str, Any]:
    cmd = ["issue", "create", "--repo", args.repo, "--title", args.title, "--body", args.body]
    ...
    return await _invoke_gh(
        ...
        args_summary={
            ...
            "label_count": len(args.labels or []),
        },
        cmd=cmd,
        timeout=30.0,
    )
...
async def _github_issue_view(ctx: ToolContext, args: GithubIssueViewArgs) -> dict[str, Any]:
    cmd = [
        ...
        args.repo,
        "--json",
        "number,title,body,state,author,labels,comments,createdAt",
    ]
    return await _invoke_gh(
        ctx,
        action_type="github.issue_view",
...
async def _github_issue_comment(ctx: ToolContext, args: GithubIssueCommentArgs) -> dict[str, Any]:
    ...
    return await _invoke_gh(
        ctx,
        action_type="github.issue_comment",
        target_uri=f"{args.repo}#{args.number}",
        args_summary={
            "repo": args.repo,
            "number": args.number,
...
async def _github_issue_close(ctx: ToolContext, args: GithubIssueCloseArgs) -> dict[str, Any]:
    ...
    return await _invoke_gh(
        ctx,
        action_type="github.issue_close",
        target_uri=f"{args.repo}#{args.number}",
        args_summary={
            "repo": args.repo,
            "number": args.number,
...
async def _github_pr_merge(ctx: ToolContext, args: GithubPrMergeArgs) -> dict[str, Any]:
    cmd = ["pr", "merge", str(args.number), "--repo", args.repo, f"--{args.strategy}"]
    if args.delete_branch:
        cmd.append("--delete-branch")
    return await _invoke_gh(
        ctx,
        action_type="github.pr_merge",
        target_uri=f"{args.repo}#{args.number}",
        args_summary={
            "repo": args.repo,
            "number": args.number,
...
async def _github_workflow_list(
    ctx: ToolContext,
    args: GithubWorkflowListArgs,
) -> dict[str, Any]:
    cmd = ["workflow", "list", "--repo", args.repo, "--json", "id,name,state,path"]
    return await _invoke_gh(
        ctx,
        action_type="github.workflow_list",
        target_uri=args.repo,
        args_summary={"repo": args.repo},
        cmd=cmd,
        timeout=30.0,
...
def build_github_tools_inner() -> list[ToolSpec[Any]]:
    return [
        ...
        ToolSpec(
            ...
            args_model=GithubRepoViewArgs,
            handler=_github_repo_view,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_repo_clone",
            description="Clone a GitHub repo to a host directory.",
            args_model=GithubRepoCloneArgs,
            handler=_github_repo_clone,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_repo_fork",
            description="Fork a GitHub repo (no clone; pass --org to fork into an org).",
            args_model=GithubRepoForkArgs,
            handler=_github_repo_fork,
            defer_loading=True,
        ),
        ToolSpec(
            ...
            args_model=GithubRepoCreateArgs,
        ...
        ToolSpec(
            ...
            args_model=GithubIssueViewArgs,
            handler=_github_issue_view,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_issue_comment",
            description="Add a comment to a GitHub issue.",
            args_model=GithubIssueCommentArgs,
            handler=_github_issue_comment,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_issue_close",
            description="Close a GitHub issue (optionally with a closing comment).",
            args_model=GithubIssueCloseArgs,
            handler=_github_issue_close,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_pr_list",
            ...
            handler=_github_pr_list,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_pr_view",
            description="View a GitHub PR metadata + review state as JSON.",
            args_model=GithubPrViewArgs,
            handler=_github_pr_view,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_pr_merge",
            description=(
                "Merge a GitHub PR via merge / squash / rebase strategy. T2-risk; warden gates."
            ),
            args_model=GithubPrMergeArgs,
            handler=_github_pr_merge,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_workflow_list",
            description="List GitHub Actions workflows in a repo.",
            args_model=GithubWorkflowListArgs,
            handler=_github_workflow_list,
            defer_loading=True,
        ),
        ToolSpec(
            name="github_workflow_run",
            description="Trigger a GitHub Actions workflow_dispatch by name/ID + ref.",
            args_model=GithubWorkflowRunArgs,
            handler=_github_workflow_run,
            defer_loading=True,
        ),
    ]
