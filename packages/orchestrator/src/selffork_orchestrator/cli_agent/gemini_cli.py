"""GeminiCliAgent — planned stub.

Implementation lands in **M2-M3** per
``project_selffork_jr_drives_3_cli_agents.md``.

When implemented, ``build_command`` MUST include the ``--yolo`` flag for
unattended runs (per ``project_per_cli_auto_approve_flags.md``).
"""
...
class GeminiCliAgent(CLIAgent):
    ...
    def __init__(self, config: CLIAgentConfig) -> None:
        raise NotImplementedError(
            "GeminiCliAgent is planned for M2-M3. For MVP, set cli_agent.agent='opencode'.",
        )
