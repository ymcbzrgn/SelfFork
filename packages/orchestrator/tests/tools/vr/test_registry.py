...
def test_vr_no_collision() -> None:
    ...
    other = {n for n in names if n.startswith((
        "ios_", "android_", "browser_", "desktop_", "github_", "skill_",
    ))}
...
def test_default_registry_includes_vr_fleet() -> None:
    ...
    assert len(vr) == 27
    eager = sum(
        1 for n in vr if not registry.get(n).defer_loading  # type: ignore[union-attr]
    )
    assert eager == 3
